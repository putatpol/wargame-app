...
import DebuffRemoveModal from "./DebuffRemoveModal";
...
export default function TeamSection({
  ...
  const [pendingRemove, setPendingRemove] = React.useState<{
    characterId: number;
    buffId: number;
    buff?: StatusBuff | undefined;
  } | null>(null);

  const handleTryRemove = (characterId: number, buffId: number) => {
    const b = statusBuffs.find((s) => s.id === buffId);
    if (b?.stat === "debuff") {
      setPendingRemove({ characterId, buffId, buff: b });
      setShowRemoveModal(true);
    } else {
      removeStatusBuff(characterId, buffId);
    }
  };
